# -*- coding: utf-8 -*-
"""
@author: behmardaida, 12/19/2018

- Creates control sample for snr and resolution degradation tests
"""
...
# pickle things
pickling_on = open('cannon2_specmatch_labels_cntrl.pkl', "wb")
...
pickling_on = open('cannon2_specmatch_spectra_cntrl.pkl', "wb")
pickle.dump(spectra_val_noNa,pickling_on)
...
for _ in range(20):
        ...
        cannon_labels_tr, cov_tr, metadata_tr = model.test(spectra_tr_noNa, ivar_train)

        # cannon-derived labels for validation set
        cannon_labels_val, cov_val, metadata_val = model.test(spectra_val_noNa, ivar_validate)
...
# --------------------------- pickles things --------------------------------------
# Cannon-derived flux
pickling_on = open('cannon2_cannon_spectra_cntrl.pkl', "wb")
pickle.dump(cannon_spectra,pickling_on)

# Cannon-derived parameters
pickling_on = open('cannon2_cannon_labels_cntrl.pkl', "wb")
pickle.dump(cannon_labels,pickling_on)

# Cannon model intrinsic variance
pickling_on = open('cannon2_s2_cntrl.pkl', "wb")
