...
- Resolution and SNR degradation test
"""
...
# ---- enter S/N degradation level ---------------------------
target_snr = int(input("Enter desired SNR: "))
def sigma_snr(star_snr):
        sigma = np.sqrt((1/target_snr)**2 - (1/star_snr)**2)
        # deals with S/N edge case
...
for spectrum1,val1 in zip(spectra_tr,err_tr):
        spectrum1,va1l = gauss_norm(spectrum1,lib.wav,val1,3)
...
# validation set
spectra_val_noNa_degrad2=[]
err_val_noNa_degrad=[]
for spectrum2,err2 in zip(spectra_val_noNa_degrad,err_val_noNa):
        spectrum2 = gaussian_filter1d(spectrum2,sigma,mode='constant')
        spectrum2_rebin = rebin(spectrum2)
        spectra_val_noNa_degrad2.append(spectrum2_rebin)

        err2_rebin = rebin(err2)
        err_val_noNa_degrad.append(err2_rebin)

spectra_val_noNa_degrad2 = np.asarray(spectra_val_noNa_degrad2)
err_val_noNa_degrad = np.asarray(err_val_noNa_degrad)


pickling_on = open('cannon2_specmatch_spectra_res' + str(R) + '.pkl', "wb")
pickle.dump(spectra_val_noNa_degrad2,pickling_on)



print(spectra_val_noNa_degrad2.shape)
print(spectra_tr_noNa_degrad.shape)

print(err_val_noNa_degrad.shape)
print(err_tr_noNa_degrad.shape)
...
for _ in range(20):
        ...
        cannon_labels_val, cov_val, metadata_val = model.test(spectra_val_noNa_degrad2, ivar_validate)
...
# --------------------------- pickles things --------------------------------------
# Cannon-derived flux
pickling_on = open('cannon2_cannon_spectra_res' + str(R) + 'snr' + str(target_snr) + .pkl', "wb")
pickle.dump(cannon_spectra,pickling_on)

# Cannon-derived parameters
pickling_on = open('cannon2_cannon_labels_res' + str(R) + 'snr' + str(target_snr) + '.pkl', "wb")
pickle.dump(cannon_labels,pickling_on)

# Cannon model intrinsic variance
pickling_on = open('cannon2_s2_res' + str(R) + 'snr' + str(target_snr) + '.pkl', "wb")
